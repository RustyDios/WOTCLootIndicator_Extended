...
var config bool SHOW_LOOT, REQUIRE_SCANNING, SHOW_STATS_ON_FRIENDLY, SHOW_BARS_ON_FRIENDLY, SHOW_MAX_HP, SHOW_FRIENDLY_NAME, SHOW_ENEMY_NAME, NAME_COLOR_BYTEAM, HPBAR_COLOR_BYTEAM, SHIELDBAR_COLOR_BYTEAM_FRIENDLIES, SHIELDBAR_COLOR_BYTEAM_ENEMIES;
var config bool SHOW_DAMAGE, SHOW_AIM, SHOW_DEF, SHOW_MOB, SHOW_WILL, SHOW_HACK, SHOW_DODGE, SHOW_PSI, SHOW_HP, SHOW_ARMOR, SHOW_SHIELD, SHOW_WILL_BAR, SHOW_HUDHEAD, SHOW_MOB_AS_TILES, bRustyUIFlagLog;
...
simulated function SetHitPoints( int _currentHP, int _maxHP )
{
	...
	else
	{
		...
		//IF UNIT IS NOT DEAD SET UP TO SHOW HEALTH
	...
	{
		if (UnitState.GetMyTemplateName() == 'ViperKing' 	|| UnitState.GetMyTemplateName() == 'BerserkerQueen' || UnitState.GetMyTemplateName() == 'ArchonKing' 	|| UnitState.GetMyTemplateName() == 'CXQueen' 
		 || UnitState.GetMyTemplateName() == 'ViperPrince1' || UnitState.GetMyTemplateName() == 'ViperPrince2' 	 || UnitState.GetMyTemplateName() == 'ViperPrince3' || UnitState.GetMyTemplateName() == 'ViperPrincess' 
		 || UnitState.bIsSpecial )
		{
			AS_SetMCColor(MCPath $".healthMeter.theMeter", "ACD373" );	//set ruler colour, game takes care of 'the health array extra lines' to colour to this colour
		}
...
//=============================================================
//	SHIELDS PREVIEW -- SLIGHT TWEAK AS THIS WAS GETTING 'STUCK' ON
//	COULDN'T FIX IT :( REVERTED TO GAME DEFAULT :(
//=============================================================
/*
simulated function SetShieldPointsPreview(optional int _iPossibleDamage = 0)
{
	...
	local float prebarlength, maxbarlength, curbarlength;

	local GFxObject ShieldMeter_gfxo, ShieldPreview_gfxo;

	iMultiplier = `GAMECORE.HP_PER_TICK;

	ShieldPreview_gfxo = Movie.GetVariableObject(MCPath $".healthMeter.shieldMeter.thePreview");	//loc2	//the flash yellow
	ShieldMeter_gfxo = Movie.GetVariableObject(MCPath $".healthMeter.shieldMeter.theMeter");		//loc3	//the normal yellow
	...
	//DO THE MATH
	maxbarlength = m_shieldPoints.GetValue() / m_maxShieldPoints.GetValue() * 198; //reset length
	prebarlength = m_shieldPointsPreview.GetValue() / m_maxShieldPoints.GetValue() * 198; //loc 6 
	curbarlength = (m_shieldPoints.GetValue() - m_shieldPointsPreview.GetValue() ) / m_maxShieldPoints.GetValue() * 198; //loc 5

	if( m_shieldPointsPreview.HasChanged() )
	{
		//myValue.Type = AS_Number;

		//myValue.n = m_shieldPointsPreview.GetValue(); //iPossibleDamage;
		//myArray.AddItem(myValue);

		//Invoke("SetShieldPointsPreview", myArray);

		if (m_shieldPointsPreview.GetValue() > 0.0f ) //damage 
		{
			ShieldPreview_gfxo.SetBool("_visible", true); 
			ShieldPreview_gfxo.SetFloat("_width", prebarlength + curbarlength);  
			ShieldPreview_gfxo.GotoAndPlay("_animate");

			ShieldMeter_gfxo.SetBool("_visible", true); 
			if (curbarlength != 0)
			{
				ShieldMeter_gfxo.SetFloat("_width", curbarlength ); 
			}
			else
			{
				ShieldMeter_gfxo.SetFloat("_width", 1); 
			}
		}
		else // no damage ... reset ... <= 0.0f
		{
			ShieldPreview_gfxo.SetBool("_visible", false);
			ShieldPreview_gfxo.SetFloat("_width", maxbarlength );
			ShieldPreview_gfxo.GotoAndPlay("_default");				

			ShieldMeter_gfxo.SetBool("_visible", true);
			ShieldMeter_gfxo.SetFloat("_width", maxbarlength );
			ShieldMeter_gfxo.GotoAndStop("max");
		}
	}
}
*/
...
/*
FLASH FUNCTION	OVERRIDEN BY NEVER CALLING IT !!
	function SetShieldPointsPreview(potential)
	{
		...
		else
		{
			...
			if(_loc7_ < 0)
			{
				...
				_loc3_._visible = false; 		<<<<<<<<< pretty sure this is part of the 'stuck on' issue
			}
			else
			{
			...
			_loc2_._visible = true;				<<<<<<<<< pretty sure this is part of the 'stuck on' issue
			_loc2_.gotoAndPlay("_animate");		<<<<<<<<< pretty sure this is part of the 'stuck on' issue
		}
	}
*/
...
function int GetYShift() 
{
	...
	// BUMP UP IF IT IS AN OBJECT, ODDLY ENOUGH ALWAYS THE SAME SHIFT NEEDED FOR THE WILL BAR
...
// ALSO EXTENDED TO ELITE VIPERS AND ABA VIPERS
//=============================================================
